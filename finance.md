...
## [置信系数模型]
我们可以计算出来一个权重矩阵，然后使用市场参数矩阵*权重矩阵的列向量来计算一个在债券收益率的目标值
最后根据目标值来估算落在不同的区间的概率，之后就是给定区间算置信度 或者给定置信度算区间，一个可以推演到偏好风险上去，一个可以映射到厌恶风险上去





[dr007 and r007 ](https://www.zhihu.com/question/265870524)
...
## [七日年化收益率](https://jingyan.baidu.com/article/d8072ac4411b64ec94cefd7e.html)

从目前来看，债券的流动性一般是指好不好倒手。
...
### [建设银行](http://gold1.ccb.com/cn/gold/20090417_1239953346.html)

## 交易的时效性是个问题

如果手续费是0.8元一克的话，那么单天黄金需要上涨0.8元我才能有获利的可能
...
# 房地产价格模型
短期看金融，中期看土地,长期看人口，总体上按照供需来
## 基本概念
[成交建筑面积/楼面价](http://cq.news.fang.com/open/30239281.html)
[受让人](http://www.sohu.com/a/273558757_395088)
[楼面价/成交价/容积率](http://news.officese.com/2018-1-16/85115.html)
...
## 从供应和需求来看我们需要关注以下指标
### 人口数量
1.该城市购买婴儿尿不湿的数量-------对应的新生人口数量以及以及家庭购买房产的愿望
2.该城市热点区域房租价格可以反映外来人口流入量
3.该城市出现在热搜的次数
4.不断出现的80，80，00后，以及在线大学生数量
...
1.趋势模型------------------设定一个定义阈值来检测某一段的危险性
  ^ - ^
    |
    |
 事件驱动模型（新闻好或坏）------	>某一方面的交易量或
  最活跃的
 次活跃的现券
   |||
    |
...
网贷也许是个好的方向,我们可以依赖房地产商的数据获取能力，使用线性回归来进行计算
...
### 其他数据来源
1.[中国二手房指数](http://www.cchindex.com/Home/Index)
2.
...
9.现券的交易数据
10.加入货币政策
11.一瞬间的消息看盘中的反应，如果处在一个大背景下，我主要判断一个大背景
12.债券价格迅速上扬，出现在大背景的初期，中期和长期
13.如果从盘中的结果来看是好消息还是坏消息，拉长了去关注趋势的问题，如果关注日数据
来判断大趋势
14.好消息和预期
15.二阶导数
16.找出该趋势处于的部位
